...
def parse_law(url: str, languages: tuple[tuple[str, str, str], ...]) -> None:
    """Fetch every language version of a fedlex law and save its entries.

    `languages` is a tuple of `(lang, prompt, warning)` triples. Each prompt must
    contain a `%ID%` placeholder that is replaced with the article number.
    """
